...
% 1 for the sign, 8 for quantizing (logarithmically) its absolute value.
% Optionally, the maximum value ("hot corner") is stored aside (as it tends to be
% really large).
% This strategy was first used in "Interactive Multiscale Tensor Reconstruction for
% Multiresolution Volume Visualization" (S. Suter et al.)
%
...
function [X_quantized, maximum] = log_quantize(X,assume_hot_corner)
    ...
    X_quantized(start:end) = 255*X_quantized(start:end);
